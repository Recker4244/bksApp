...
class _HomeState extends State<Home> {
  ...
  Future getSlider() async {
    ...
    var request = http.Request(
        'GET', Uri.parse('https://goldv2.herokuapp.com/api/slider'));
    ...
    if (response.statusCode == 200) {
      ...
      sliders = List<slider>.from(l.map((model) => slider.fromJson(model)));
    } else {
      print(response.reasonPhrase);
    }
  }
                                      ...
                                      children: [
                                        Column(
                                          children: [
                                            ...
                                            Text(
                                              item.mode == "Value"
                                                  ? "INR ${item.cyclePeriod.minValue}/ ${item.cyclePeriod.shortName}"
          ...
          if (snapshot.hasData) {
            List<Widget> item = [
              ...
              CarouselSlider.builder(
                ...
                itemBuilder: (BuildContext context, int index, int k) {
                  return Padding(
                    ...
                    child: Stack(
                      children: <Widget>[
                        Container(
                            ...
                            child: ClipRRect(
                                borderRadius: BorderRadius.circular(20.0),
                                child: Image.network("${sliders[index].image}",
                                    fit: BoxFit.fitWidth))),
                      ],
                    ),
                  );
