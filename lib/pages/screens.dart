export 'bottom_bar.dart';
export 'home/home.dart';
export 'home/guest_home.dart';
export 'market/market.dart';
export 'Eshop/gold_coin.dart';
export 'market/forfieted.dart';
export 'market/completed.dart';
export 'market/running.dart';
export 'market/plans.dart';
export 'package:gold247/language/locale.dart';
export 'instantGold/currency_screen.dart';
export 'instantGold/crypto_chart_syncfusion.dart';
export 'buySccessFailScreen/buy_success_screen.dart';
export 'buySccessFailScreen/buy_fail_screen.dart';
export 'bankSuccessFailScreen/bank_fail_screen.dart';
export 'bankSuccessFailScreen/bank_success_screen.dart';
export 'portfolio/portfolio.dart';
export 'portfolio/total_balance.dart';
export 'portfolio/buy.dart';
export 'portfolio/sell_redeem.dart';
export 'profile/profile.dart';
export 'profile/edit_profile.dart';
export 'profile/support.dart';
export 'profile/privacy_policy.dart';
export 'profile/bank_details.dart';
export 'auth/login.dart';
export 'auth/otp_screen.dart';
export 'auth/register.dart';
export 'splashScreen.dart';
